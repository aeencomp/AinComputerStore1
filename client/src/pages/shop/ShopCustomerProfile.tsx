...
export default function ShopCustomerProfile() {
  ...
  const { data: customer, isLoading: isCustomerLoading } = useQuery<SaasRepairCustomer>({
    queryKey: ['/api/saas/customers/id', customerId],
    enabled: !!customerId,
    retry: false,
  });

  const { data: tickets, isLoading: isTicketsLoading } = useQuery<SaasRepairTicket[]>({
    queryKey: ['/api/saas/customers', customer?.id, 'tickets'],
    queryFn: async () => {
      const res = await fetch(`/api/saas/customers/${customer!.id}/tickets`);
  ...
  const handleJump = () => {
    ...
    navigate(`/shop/customer/${normalized}`);
    setJumpId('');
  };
  ...
  if (!customer) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 p-4" dir={isRTL ? 'rtl' : 'ltr'}>
        ...
        <Button variant="outline" onClick={() => navigate('/shop/dashboard')} data-testid="button-back-dashboard">
      ...
      <div className="max-w-4xl mx-auto p-4 space-y-6">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-3 min-w-0">
            <Button variant="ghost" size="icon" onClick={() => navigate('/shop/dashboard')} data-testid="button-back">
