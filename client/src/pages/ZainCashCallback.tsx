...
export default function ZainCashCallback() {
  ...
  const { data: paymentData } = useQuery<PaymentStatusResponse>({
    queryKey: ['/api/zaincash/status', orderNumber],
      ...
      case 'completed':
        return {
          ...
          description: language === 'ar' 
            ? 'تم استلام دفعتك بنجاح وسيتم معالجة طلبك قريباً.'
  ...
  const getErrorTranslation = (error: string): string => {
    const translations: Record<string, { ar: string; en: string }> = {
      'no_token': { ar: 'لم يتم استلام رمز التحقق', en: 'No verification token received' },
      'invalid_token': { ar: 'رمز التحقق غير صالح', en: 'Invalid verification token' },
